...
export default function PersonalizedPlan({ doshaResult, symptoms = [] }: PersonalizedPlanProps) {
  ...
  const getSymptomRecommendations = (symptoms: string[]): Array<{text: string, image?: string}> => {
    ...
    if (symptoms.includes('Anxiety')) {
      recommendations.push({text: 'Add 10 min meditation or deep breathing before bed (calms vata)', image: '/images/wellness/meditation.jpg'});
    ...
    if (symptoms.includes('Insomnia')) {
      ...
      recommendations.push({text: 'Warm milk with nutmeg 30 min before sleep', image: '/images/herbs/warm-milk.jpg'});
    }
    ...
    if (symptoms.includes('Indigestion') || symptoms.includes('Acidity')) {
      ...
      recommendations.push({text: 'Drink ginger tea 20 min before meals', image: '/images/herbs/ginger-root.jpg'});
    }

    if (symptoms.includes('Fatigue') || symptoms.includes('Low Immunity')) {
      recommendations.push({text: 'Consider Ashwagandha (300mg) in warm milk before bed', image: '/images/herbs/ashwagandha.jpg'});
      recommendations.push({text: 'Prioritize 7-8 hours sleep - this rebuilds ojas (vitality)', image: '/images/wellness/sleep.jpg'});
    }

    if (symptoms.includes('Constipation')) {
      recommendations.push({text: '1 tbsp ghee in warm water first thing in morning', image: '/images/herbs/ghee.jpg'});
    ...
    if (symptoms.includes('Joint Pain')) {
      ...
      recommendations.push({text: 'Drink turmeric milk (½ tsp turmeric in warm milk)', image: '/images/herbs/turmeric.jpg'});
    }
              ...
              {getSymptomRecommendations(userSymptoms).map((rec, idx) => (
                <div key={idx} className="flex items-start bg-[var(--background)] bg-opacity-50 p-3 rounded-lg">
                  ...
                  <div className="flex-1 flex items-center gap-3">
                    ...
                    {rec.image && (
                      <div className="w-16 h-16 rounded-lg overflow-hidden border-2 border-[var(--accent-primary)] flex-shrink-0">
                        <Image
                          src={rec.image}
                          alt="Remedy"
                          width={64}
